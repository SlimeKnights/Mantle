...
public class FluidRenderer {
  ...
  public static void putTexturedQuad(VertexConsumer renderer, PoseStack.Pose pose, TextureAtlasSprite sprite, Vector3f from, Vector3f to, Direction face, int color, int brightness, int rotation, boolean flowing) {
    ...
    if ((rotation % 180) == 90) {
      minU = (float) sprite.getU(v1 * size);
      maxU = (float) sprite.getU(v2 * size);
      minV = (float) sprite.getV(u1 * size);
      maxV = (float) sprite.getV(u2 * size);
    } else {
      minU = (float) sprite.getU(u1 * size);
      maxU = (float) sprite.getU(u2 * size);
      minV = (float) sprite.getV(v1 * size);
      maxV = (float) sprite.getV(v2 * size);
    }
