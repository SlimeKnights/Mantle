...
public class TagPreference {
  ...
  public static <T> Optional<T> getPreference(TagKey<T> tag) {
    // Only cache successful lookups. Some recipes ask for tag outputs before tags
    // finish syncing; caching Optional.empty there breaks later recipe packet encoding.
    Optional<T> cached = (Optional<T>) PREFERENCE_CACHE.get(tag);
    if (cached != null) {
      return cached;
    }
    Optional<T> preference = getUncachedPreference(tag);
    if (preference.isPresent()) {
      PREFERENCE_CACHE.put(tag, preference);
    }
    return preference;
  }
