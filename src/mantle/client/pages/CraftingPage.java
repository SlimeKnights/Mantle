...
public class CraftingPage extends BookPage
{
    ...
    public void drawCraftingPage(String info, ItemStack[] icons, int recipeSize, int localWidth, int localHeight)
    {
        if (info != null)
        {
            this.manual.fonts.drawString("\u00a7n" + info, localWidth + 50, localHeight + 4, 0);
        }
        ...
        if (recipeSize == 2)
        {
            this.manual.renderitem.renderItemAndEffectIntoGUI(icons[0], (localWidth + 126) / 2, (localHeight + 68) / 2);
            if (icons[0].stackSize > 1)
            {
                this.manual.renderitem.renderItemOverlayIntoGUI(this.manual.fonts, icons[0], (localWidth + 126) / 2, (localHeight + 68) / 2, String.valueOf(icons[0].stackSize));
            }
            for (int i = 0; i < icons.length - 1; i++)
            {
                if (icons[i + 1] != null)
                {
                    this.manual.renderitem.renderItemAndEffectIntoGUI(icons[i + 1], (localWidth + 14 + 36 * (i % 2)) / 2, (localHeight + 36 * (i / 2) + 52) / 2);
                }
            }
        }
        ...
        if (recipeSize == 3)
        {
            this.manual.renderitem.renderItemAndEffectIntoGUI(icons[0], (localWidth + 138) / 2, (localHeight + 70) / 2);
            if (icons[0].stackSize > 1)
            {
                this.manual.renderitem.renderItemOverlayIntoGUI(this.manual.fonts, icons[0], (localWidth + 126) / 2, (localHeight + 68) / 2, String.valueOf(icons[0].stackSize));
            }
            for (int i = 0; i < icons.length - 1; i++)
            {
                if (icons[i + 1] != null)
                {
                    this.manual.renderitem.renderItemAndEffectIntoGUI(icons[i + 1], (localWidth - 2 + 36 * (i % 3)) / 2, (localHeight + 36 * (i / 3) + 34) / 2);
                }
            }
        }
